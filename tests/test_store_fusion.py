...
записано в docs/CAPTION_SEARCH_C0.md. Здесь проверяется, что рантайм делает ровно
...
def test_caption_lifts_matching_photo(store):
    ...
    Первого места здесь не требуется, и это не послабление теста: при весе 0,6
...
def test_default_alpha_matches_measurement():
    """Вес не должен разъехаться с тем, что записано в отчёте C0."""
    assert CAPTION_FUSION_ALPHA == 0.6
